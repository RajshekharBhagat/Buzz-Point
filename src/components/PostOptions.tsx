...
import { useDeletePostById, useIsPostSaved, useSavePostById, useUnsavePostById } from "@/hooks/use-post";
...
const PostOptions = ({ postId, author, hiveName }: PostOptionsProps) => {
  ...
  const {mutate: deletePost, isPending: isDeletePending} = useDeletePostById(postId,hiveName)
  const {mutate: savePost, isPending: isSavePending} = useSavePostById(postId);
  const {mutate: unSavePost, isPending: isUnsavePending} = useUnsavePostById(postId);
