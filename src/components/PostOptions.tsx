...
const PostOptions = ({ postId, author }: PostOptionsProps) => {
  const { data: session } = useSession();
  const { toast } = useToast();
  const {loginToast} = useCustomToast();
  
  const {data: isSaved, refetch} = useQuery({
    queryKey: ["isPostSaved"],
    queryFn: async() => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="bg-white w-[10rem]">
        <DropdownMenuItem onClick={handlePostSave} disabled={isSavePending || isUnSavePending} className="flex items-center gap-3">
          <SaveIcon />
